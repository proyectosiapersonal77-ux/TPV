...
import { Lightbulb, Calculator, TrendingUp, AlertTriangle, RefreshCw, ChevronRight, Sparkles, ChevronLeft, X } from 'lucide-react';
...
export const MenuEngineeringScreen: React.FC<Props> = ({ onNavigate }) => {
    ...
    const handleApplyMassPricing = async () => {
        if (!confirm(`¿Estás seguro de que quieres actualizar el PVP de ${filteredProducts.length} productos para alcanzar un margen del ${targetMargin}%?`)) return;
    ...
    const handleGenerateAI = async () => {
        ...
        try {
            ...
            if (dataToAnalyze.length === 0) {
                alert("No hay productos compuestos (recetas) para analizar en esta categoría.");
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-3">
                                <input 
                                    ...
                                    min="1" 
                                    max="99"
                                    className="w-full bg-brand-900 border border-brand-600 rounded-lg p-3 text-white outline-none focus:border-brand-accent text-xl font-bold"
                                    value={targetMargin}
                                ...
                                <button 
                                    ...
                                    className="bg-brand-accent hover:bg-brand-accentHover text-white px-6 py-3 rounded-lg font-bold transition-colors disabled:opacity-50 flex items-center gap-2 whitespace-nowrap"
                                >
                                    {isCalculating ? <RefreshCw className="animate-spin" size={20} /> : <TrendingUp size={20} />}
                                    Aplicar a {filteredProducts.length} prod.
                                </button>
                            </div>
                        </div>
                    </div>
