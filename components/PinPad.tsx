...
const PinPad: React.FC<PinPadProps> = ({ onSuccess, isLoading, error, clearError }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full max-w-md mx-auto p-4 lg:p-0">
      ...
      <div className="bg-brand-800/80 backdrop-blur-md p-8 rounded-3xl shadow-2xl border border-brand-700 w-full relative overflow-hidden transition-colors duration-300">
        ...
        <div className="mb-6 mt-2 text-center h-8 flex items-center justify-center gap-2">
           {isLoading ? (
        ...
        <div className="w-full mb-8 relative flex justify-center">
          <div className={`h-14 px-8 bg-brand-900/50 rounded-full flex items-center justify-center space-x-4 border transition-colors duration-300 ${error ? 'border-red-500/50' : 'border-brand-700/50'}`}>
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
          ...
          {/* Error Message */}
          <div className={`absolute -bottom-8 left-0 w-full text-center transition-all duration-300 ${error ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2'}`}>
        ...
        <div className="grid grid-cols-3 gap-4">
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
            <button
              ...
              disabled={isLoading}
              className="h-20 bg-brand-700/50 hover:bg-brand-600/50 active:bg-brand-600 backdrop-blur-sm rounded-xl text-3xl font-light text-white transition-all shadow-sm active:scale-95 border border-brand-600/20 touch-manipulation select-none disabled:opacity-50 disabled:active:scale-100"
            >
              {num}
            </button>
          ))}
          ...
          <button
            ...
            onClick={handleClear}
            disabled={isLoading}
            className="h-20 bg-brand-800/50 hover:bg-brand-700/50 rounded-xl text-sm font-bold text-gray-400 transition-all active:scale-95 border border-brand-700/30 uppercase touch-manipulation select-none disabled:opacity-50 disabled:active:scale-100"
          >
            C
          </button>
          ...
          <button
            ...
            disabled={isLoading}
            className="h-20 bg-brand-700/50 hover:bg-brand-600/50 backdrop-blur-sm rounded-xl text-3xl font-light text-white transition-all shadow-sm active:scale-95 border border-brand-600/20 touch-manipulation select-none disabled:opacity-50 disabled:active:scale-100"
          >
            0
          </button>
          ...
          <button
            ...
            disabled={isLoading}
            className="h-20 bg-brand-800/50 hover:bg-brand-700/50 rounded-xl flex items-center justify-center text-gray-400 transition-all active:scale-95 border border-brand-700/30 touch-manipulation select-none disabled:opacity-50 disabled:active:scale-100"
          >
            <Delete size={28} />
          </button>
        </div>
