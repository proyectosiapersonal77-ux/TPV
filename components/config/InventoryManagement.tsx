                    ...
                    {activeTab === 'products' && (
                      <>
                        ...
                        <div className="grid grid-cols-3 gap-4 border-t border-brand-700 pt-4">
                            <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1">Coste ({productForm.is_compound ? 'Calc.' : 'Manual'})</label><input type="number" step="0.01" className={`w-full bg-brand-900 border border-brand-600 rounded-lg p-2 text-white text-right ${productForm.is_compound ? 'opacity-50 cursor-not-allowed' : ''}`} value={productForm.cost_price} readOnly={productForm.is_compound} onChange={e => !productForm.is_compound && setProductForm({...productForm, cost_price: parseFloat(e.target.value) || 0})} /></div>
                            <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1">IVA (%)</label><input type="number" className="w-full bg-brand-900 border border-brand-600 rounded-lg p-2 text-white text-right" value={productForm.tax_rate} onChange={e => setProductForm({...productForm, tax_rate: parseFloat(e.target.value) || 0})} /></div>
                            <div><label className="block text-xs font-bold text-brand-accent uppercase mb-1">PVP Venta (€)</label><input type="number" step="0.01" className="w-full bg-brand-900 border-brand-accent border rounded-lg p-2 text-white text-right font-bold" value={productForm.selling_price} onChange={e => setProductForm({...productForm, selling_price: parseFloat(e.target.value) || 0})} /></div>
                        </div>

                        {!productForm.is_compound && (
                            <div className="bg-brand-900/50 p-4 rounded-xl border border-brand-700 flex items-center justify-between">
                                <div><label className="text-sm font-bold text-white flex items-center gap-2"><Layers size={16} className="text-brand-accent" /> Variantes / Formatos de Venta</label><p className="text-xs text-gray-400">Si vendes este mismo producto en varios tamaños (ej. Caña, Pinta) con precios distintos.</p></div>
                                <div className="relative inline-block w-12 h-6 transition duration-200 ease-in-out rounded-full cursor-pointer"><input type="checkbox" className="absolute w-6 h-6 opacity-0 cursor-pointer" checked={productForm.hasVariants} onChange={(e) => setProductForm({...productForm, hasVariants: e.target.checked})} /><div className={`w-11 h-6 rounded-full shadow-inner transition-colors ${productForm.hasVariants ? 'bg-brand-accent' : 'bg-gray-700'}`}></div><div className={`absolute w-4 h-4 bg-white rounded-full shadow inset-y-1 transition-transform ${productForm.hasVariants ? 'translate-x-6' : 'translate-x-1'}`}></div></div>
                            </div>
                        )}
